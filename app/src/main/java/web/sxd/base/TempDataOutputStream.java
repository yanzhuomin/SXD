...
public final class TempDataOutputStream extends DataOutputStream
{
    ...
    /**
    * @param outputstream 类型 ： 主界面：0   仙界：0x5e0000  圣域：0x1250000 全网：0x150000c
    *
    * */
    public final void send(OutputStream outputstream)  throws IOException
    {
        ...
        b.posToEnd();
        if(!flag)
            writeInt(lastFuncCode[messageTag]);
        lastFuncCode[messageTag] = funcCode;
        flush();
...
//    }
    ...
    public TempDataOutputStream setMessageTag(int tag){
        messageTag = tag;
        return this;
    }
    ...
    private int  messageTag;//消息目标，主界面：0   仙界：1  圣域：2 全网：3
    private static int lastFuncCode[] = new int[4]; //上次的功能码
    int funcCode;
